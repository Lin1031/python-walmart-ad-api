class AdvertisingApiException(Exception):
    def __init__(self, code, error, headers):
        self.walm_code = error.get('code')
        self.message = error.get('details') or error.get('Message') or error.get('message') or error.get('description')
        self.code = code
        self.error = error
    ...
    def get_walm_code(self):
        return self.walm_code
...
def get_exception_for_content(content: object):
    return {
        'InvalidParameterValue': AdvertisingApiInvalidParameterValueException
    }.get(422, AdvertisingApiException)
